...
export const getApiRampShow = async (isMainnet: boolean, isFetch = true) => {
  if (isFetch) {
  ...
  const isBuyShowNew = Object.keys(rampProviders).some((key) => {
    return rampProviders[key as IRampProviderType]?.providerInfo.coverage.buy === true;
  });
  const isSellShowNew = Object.keys(rampProviders).some((key) => {
    return rampProviders[key as IRampProviderType]?.providerInfo.coverage.sell === true;
  });
  const isRampShowNew = isBuyShowNew || isSellShowNew;

  return {
    isRampShow: isMainnet && isRampShowNew,
    isBuyShow: isMainnet && isBuyShowNew,
    isSellShow: isMainnet && isSellShowNew,
  };
};
