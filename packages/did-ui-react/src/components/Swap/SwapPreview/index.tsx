...
} from '../../../hooks/awaken/state';
import { LANG_MAX, ONE, TEN_THOUSAND, ZERO } from '../../../constants/misc';
import { bigNumberToString, getDeadline, minimumAmountOut } from '../../../utils/awaken';
import { formatNameWithNoUnderline, formatPriceUsd } from '../../../utils/format';
...
import { AWAKEN_DEFAULT_CID } from '../../../constants/awaken';
...
export const SwapPreview = ({ swapInfo: swapInfoProp, swapRoute, priceLabel, onFinish }: TSwapPreviewProps) => {
  ...
  const defaultToken = useDefaultToken(dAppChainId);
  ...
  const handlePress = useCallback(async () => {
    ...
    try {
      ...
      if (valueInAmountBN.gt(allowance)) {
        ...
        const approveResult = await caContract.callSendMethod('ManagerApprove', managerAddress, {
          caHash: caHash,
          spender: swapHookContractAddress,
          symbol: tokenIn.symbol,
