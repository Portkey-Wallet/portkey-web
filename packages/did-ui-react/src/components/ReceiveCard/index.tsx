...
export interface ReceiveCardProps {
  toInfo: QRCodeDataObjType['toInfo'];
...
export default function ReceiveCard({
  ...
  const value: QRCodeDataObjType = useMemo(
    () => ({
      ...
      toInfo: {
        ...toInfo,
        address: getEntireDIDAelfAddress(toInfo.address, 'ELF', chainId),
      },
      assetInfo,
      address: getEntireDIDAelfAddress(toInfo.address, 'ELF', chainId),
    }),
    [assetInfo, chainId, networkType, toInfo],
  );
  return (
      ...
      <div className="portkey-ui-receive-content">
        ...
        <div className="portkey-ui-flex-column-center portkey-ui-receive-inner">
          ...
          <div className="receive-address">
            <div className="address">{getEntireDIDAelfAddress(toInfo.address, 'ELF', chainId)}</div>
            <Copy toCopy={getEntireDIDAelfAddress(toInfo.address, 'ELF', chainId)} />
          </div>
        </div>
      </div>
    </div>
