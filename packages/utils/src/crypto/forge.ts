...
export class ForgeCryptoManagerHex extends ForgeCryptoManager implements ICryptoManager {
  constructor() {
    super();
  }
   ...
   */
  public encryptHex = async (cryptoKey: string, data: string): Promise<string> => {
    const hexData = Buffer.from(data).toString('hex');
    return this.encrypt(cryptoKey, hexData);
  };
   ...
   * @param cryptoKey - `JsonWebKey` generated by `generateKeyPair`
   * @param data - encrypted data
   * @returns decrypted data in hex format
   */
  public decryptHex = async (cryptoKey: string, dataHex: string): Promise<string> => {
    const decodedString = await this.decrypt(cryptoKey, dataHex);
    return Buffer.from(decodedString, 'hex').toString();
  };
}
